...
global ROLES, PLAYER_COUNT, MAX_LEVEL, STARTS
...
class giveInfo(Node):
    ...
    def gen_children(self):
        ...
        possible.append("retract current role claim")
        ...
        if self.sub_level < 5: # If to deep in current player saying things, dont let them say more
            for i in possible:
                self.children.append(giveInfo(i, self.sequence, self.level, self.sub_level + 1, self.gs))
        for i in possible:
            self.children.append(terminalInfo(i, self.sequence, self.level, self.sub_level + 1, self.gs))
...
class terminalInfo(giveInfo):
    ...
    def gen_children(self):
        ...
        new_seq[1] += 1 
        if new_seq[1] == 3: # this was the final node in the subsequence
...
class AdminAction(Node):
    ...
    def gen_children(self):
        ...
        self.children.append(terminalAdmin("set happy to vote", self.sequence, self.level, self.sub_level+1, copy.deepcopy(new_game_state)))
        ...
        self.children.append(terminalAdmin("set not happy to vote", self.sequence, self.level, self.sub_level+1, copy.deepcopy(new_game_state)))
        ...
        self.children.append(terminalAdmin("has claim", self.sequence, self.level, self.sub_level+1, copy.deepcopy(new_game_state)))
    ...
    def gen_children(self):
        ...
        if new_seq[1] == 3: # this was the final node in the subsequence
            ...
            if len(for_seq) == 0:
                ...
                perms = list(permutations(for_seq))
                for i in perms: #n!
                    new_seq = [i, 0]
